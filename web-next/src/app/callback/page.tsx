...
// Thời gian tối đa cho phép xử lý callback (10 phút)
const MAX_CALLBACK_TIME = 10 * 60 * 1000;
    ...
    const handleCallback = async () => {
      try {
        ...
        if (!authStartTime || (Date.now() - parseInt(authStartTime)) > MAX_CALLBACK_TIME) {
          setStatus('error');
          setErrorMessage('Phiên xác thực đã hết hạn (quá 10 phút). Vui lòng thử lại quá trình ủy quyền.');
        ...
        // Loại 'id' và 'dataHash' khỏi body: backend (ValidationPipe forbidNonWhitelisted)
        // chỉ nhận field trong DTO; id được truyền qua URL param khi update.
        const createData = { ...accountData, authCode: code };
