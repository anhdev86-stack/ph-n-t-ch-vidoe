...
interface DataRetrievalCardProps {
  ...
  handleFetchSpecificMonth: (year: number, month: number) => void;
...
const DataRetrievalCard: React.FC<DataRetrievalCardProps> = ({
  ...
  // Specific month/year selection states
  const [selectedYear, setSelectedYear] = useState<number>(new Date().getFullYear());
  const [selectedMonth, setSelectedMonth] = useState<number>(new Date().getMonth() + 1);
  const [isLoadingSpecificMonth, setIsLoadingSpecificMonth] = useState<boolean>(false);
  ...
  // Handle fetching data for specific month/year
  const handleFetchSpecificMonthData = async () => {
    if (selectedAccountIds.length === 0) {
      message.error("Vui lòng chọn ít nhất một tài khoản!");
      return;
    }

    setIsLoadingSpecificMonth(true);
    try {
      await handleFetchSpecificMonth(selectedYear, selectedMonth);
      message.success(`Đã cập nhật dữ liệu tháng ${selectedMonth}/${selectedYear} cho ${selectedAccountIds.length} tài khoản!`);
    } catch (error) {
      console.error("Lỗi khi lấy dữ liệu tháng cụ thể:", error);
      message.error("Có lỗi xảy ra khi cập nhật dữ liệu!");
    } finally {
      setIsLoadingSpecificMonth(false);
    }
  };
        ...
        <Col xs={24} lg={12}>
          ...
          >
            <div>
              ...
              <Typography.Text strong>Lấy dữ liệu theo tháng cụ thể</Typography.Text>
              <Space direction="vertical" style={{ width: '100%', marginTop: 8 }}>
                ...
                <Button
                  ...
                  onClick={handleFetchSpecificMonthData}
                  loading={isLoadingSpecificMonth}
                  disabled={selectedAccountIds.length === 0}
                  block
                  style={{ marginTop: 8 }}
                >
                  Lấy dữ liệu tháng {selectedMonth}/{selectedYear}
                </Button>
              </Space>
